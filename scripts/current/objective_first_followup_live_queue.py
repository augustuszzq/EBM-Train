#!/usr/bin/env python3
"""Continuously evaluate checkpoint FID trajectories for objective-first follow-up runs."""
...
DEFAULT_MANIFEST = PROJECT_DIR / "experiments" / "objective_first_followup_manifest_submitted.csv"
DEFAULT_EVAL_PYTHON = "/home/kevienzzq/.conda/envs/llm-env/bin/python"
DEFAULT_STATE_JSON = Path("/eagle/lc-mpi/Zhiqing/polaris_ebm/runs_objective_first_followup/live_fid_runner_state.json")
STEP_INTERVAL = 5000
...
def build_specs(rows: Iterable[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for row in rows:
        if row.get("submit", "").strip().lower() != "yes":
            continue
        if not row.get("train_job_id", "").strip():
            continue
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser("Track objective-first checkpoint trajectories on local GPUs")
...
def main() -> int:
    ...
    while True:
        ...
        if all_done and not (set(gpu_ids) & busy_gpu_indices):
            print("[DONE] no pending objective-first trajectory rows remain")
            return 0
        if args.once:
