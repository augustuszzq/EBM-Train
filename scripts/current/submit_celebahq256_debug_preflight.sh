...
ROOT="${PROJECT_DIR}/runs_celebahq256_preflight/debug"
...
printf 'exp_id,train_mode,job_id,run_dir,manifest_row_path,submitted_at\n' > "${SUBMITTED}"
...
submit_one() {
  ...
  PIPE_STAGES="${pipe_stages}" WEIGHT_MODE="${weight_mode}" CONFIG="${config}" \
  DATA_DIR="${DATA_DIR}" /home/kevienzzq/.conda/envs/llm-env/bin/python - <<'PY'
import json
import os
import time
...
payload = {
    ...
    "group": "celebahq256_debug_preflight",
    "exp_id": os.environ["EXP_ID"],
    "story": "CelebA-HQ 256 dataset/model/runtime preflight only; not a numeric experiment",
...
PY
  ...
  qsub_vars=$(
    printf 'PROJECT_DIR=%s,GROUP=%s,EXP_ID=%s,STORY=%s,TRAIN_MODE=%s,WORLD_SIZE=%s,PIPE_STAGES=%s,K=%s,STEPS=%s,LR=%s,STEP_SIZE=%s,NOISE_STD=%s,WEIGHT_MODE=%s,LAST2_BETA=%s,SEED=%s,NUM_NODES=%s,PPN=%s,EVAL_IMAGES=%s,RUN_DIR=%s,MANIFEST_ROW_PATH=%s,SAVE_EVERY=%s,VIS_EVERY=%s,BATCH_SIZE=%s,SIGMA_PD=%s,LANGEVIN_SIGN=%s,WEIGHT_DECAY=%s,MAX_GRAD_NORM=%s,DATA_DIR=%s,CONFIG=%s,SYNC_FRESH_INIT=%s,DEBUG_LEVEL=%s,LOG_EVERY=%s' \
      "${PROJECT_DIR}" \
      "celebahq256_debug_preflight" \
      "${exp_id}" \
      "CelebA-HQ256 preflight only" \
      "${train_mode}" \
      "4" \
      "${pipe_stages}" \
      "4" \
  ...
  local job_id
  job_id="$(qsub -q debug -l select=1:system=polaris -l walltime=00:30:00 -N "chqdbg" -v "${qsub_vars}" "${PBS_SCRIPT}" | awk '{print $1}')"
  printf '%s,%s,%s,%s,%s,%s\n' "${exp_id}" "${train_mode}" "${job_id}" "${run_dir}" "${manifest_row}" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "${SUBMITTED}"
  printf '[SUBMITTED] %s %s %s %s\n' "${job_id}" "${train_mode}" "${exp_id}" "${run_dir}"
}

submit_one \
  "celebahq256_ddp_preflight_b64_k4_s5_seed1" \
  "ddp_fullk" \
  "1" \
  "deep_only" \
...
submit_one \
  "celebahq256_pipe_p4_preflight_b64_k4_s5_seed1" \
  "pipe_strict" \
  "4" \
  "uniform" \
